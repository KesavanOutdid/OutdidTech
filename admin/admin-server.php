...
$password_1 = mysqli_real_escape_string($conn, $_POST['password_1']);
 ...
 $user_check_query = "SELECT * FROM admin_user_list WHERE email = '$email' LIMIT 1";
 ...
 if($user) {
   ...
   $password = $password_1; //this will not encrypt the password
    $query ="INSERT INTO admin_user_list (name, email, password) VALUES ('$name', '$email', '$password')";
 ...
 if(count($errors) == 0 ) {
   ...
   $password = $password; //this will not encrypt the password
    ...
    $query = "SELECT * FROM admin_user_list WHERE email = '$email' AND password ='$password'";
    ...
    // echo 'alert("sessionStorage.setItem("name", "'.$name.'"));';
      echo 'window.location.href="main-index.html";';
      echo '</script>';
